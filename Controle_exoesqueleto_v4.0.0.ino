...
um motor DC com caixa de redução. Os comandos são recebidos 
por bluetooth, e consistem em: mover ('m', abre e fecha a mão), parar 
os motores ('p', pode ser usado em fase de testes para parar os 
motores em momentos indevidos) e informar a tensão da bateria ('v').
Para cada movimento, deve ser realizado a leitura dos parâmetros:
Tempo, valor de PWM inicial, e valor de PWM final. */
...
//Leituras
#define LED_DADOS         13    // Em breve não será necessário porque será indicado por efeito na fita de LEDs
...
#define PWM_MAX                   255 //Não usa mais ----- Pode apagar
#define TEMPO_MAX                 2000  //Tempo máximo do movimento ---- Em breve alterar para 1.3s
...
/**********************************************
 * Função responsável pelo movimento da órtese
 **********************************************/
void pensar(){
  int pixel = 0;
  /*
...
//  if (idx_leit_corrente){
  if (idx_leit_corrente && idx_movimento < QTD_MOVIMENTOS_MAX){   //------Zé colocou assim... talvez não seja nesse sentido. Estou com dúvida
 ...
 * -----------Totalmente diferente, alterar depois de testar as outras coisas antes ------------
 *********************************************/
void envia_medias(){
...
//      }
      ...
      //Efeito fade_in para o bloco mover:
//      if (tipo_movimento == 2 ){
